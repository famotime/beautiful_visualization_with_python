...
for a, b, label in zip(y, x_value, x_label):  # 给条形图加标签，需要使用for循环
    plt.text(0, b+0.45, s=label, ha='left', va='center',
             fontsize=13, family='sans-serif')
    plt.text(a+0.01, b, s="$" + str(round(a, 2)), ha='left',
             va='center', fontsize=13.5, family='Arial', weight="bold")

plt.text(0, 1.3, s='Economic Engine', transform=ax.transAxes,
         weight='bold', size=20, family='Arial')
...
plt.text(0, -0.05, s='Source: Daniel J. Meckstroth.MAPI Foundtation;U.S. Bureau\nof Economic Analysis',
         transform=ax.transAxes, weight='light', size=10, family='sans-serif')
