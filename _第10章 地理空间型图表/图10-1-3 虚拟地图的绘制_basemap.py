...
basemap.readshapefile(shapefile='Virtual_Map0',
                      name="Country", drawbounds=True)
...
basemap.drawparallels(np.arange(lat_min, lat_max, 10),
                      labels=[1, 0, 0, 0])  # 画经度线
basemap.drawmeridians(np.arange(lon_min, lon_max, 10),
                      labels=[0, 0, 0, 1])  # 画纬度线

patches = [mpatches.Patch(color=color[i], label=country[i])
...
basemap = Basemap(llcrnrlon=lon_min, urcrnrlon=lon_max, llcrnrlat=lat_min, urcrnrlat=lat_max,
                  projection='cyl', lon_0=120, lat_0=50, ax=ax)
basemap.readshapefile(shapefile='Virtual_Map1',
                      name="Country", drawbounds=True)
...
basemap.drawparallels(np.arange(lat_min, lat_max, 10),
                      labels=[1, 0, 0, 0], zorder=0)  # 画经度线
basemap.drawmeridians(np.arange(lon_min, lon_max, 10),
                      labels=[0, 0, 0, 1], zorder=0)  # 画纬度线
...
collection = PatchCollection(
    patches, facecolor='b', alpha=1, edgecolor='k', linewidth=1)  # ,
